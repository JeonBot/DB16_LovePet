...
if($select == 1) // 바로 결제
{
    $query100 = "SELECT MEMBER_NUM FROM BASKET WHERE MEMBER_NUM = (SELECT MEMBER_NUM FROM MEMBER WHERE ID = '$ID')";
    $result100 = mysqli_query($con, $query100);
    $row100 = mysqli_fetch_array($result100);

    $query = "SELECT PRICE FROM BASKET WHERE MEMBER_NUM = (SELECT MEMBER_NUM FROM MEMBER WHERE ID = '$ID')";
